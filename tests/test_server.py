...
def test_research_tool_preserves_structural_edge_provenance(tmp_path: Path) -> None:
    write_python(
        tmp_path,
        "service.py",
...
def test_research_tool_preserves_behavioral_argument_evidence(tmp_path: Path) -> None:
    ...
    assert {
        ...
        "reason": "passed as argument",
        "weight": 1,
        "locations": [{"path": "service.py", "line": 3, "column": 4, "argument": "value"}],
    } in data["edges"]


def test_research_tool_preserves_comparison_operator_evidence(tmp_path: Path) -> None:
    write_python(
        tmp_path,
        "service.py",
        "def compare(amount, limit):\n    return amount < limit\n",
    )
    git_repository(tmp_path, "service.py")
