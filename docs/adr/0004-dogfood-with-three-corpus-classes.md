...
usefulness. The default dogfooding set is `gitminer-dash`, `quizzology`, `OpenModelica`, `otter-kr`,
and `langchain-ai/deepagents`: a long-history reference, a medium application, a warning-heavy
edge case, the self-hosting project, and a larger multi-package Python project. Additional
repositories, including `boxed/mutmut`, may be added when a slice needs a distinct corpus
characteristic. Retain raw packets, versions, queries, warnings, and hypotheses separately.
Generated workspaces under the gitignored `dogfood/` directory are disposable; only minimized
fixtures and golden packets are versioned.
